...
// UPDATED: This route now calculates and returns all three balances
router.get("/balances", async (req, res) => {
  try {
    ...
    // Calculate total spent by users by summing the 'charge' from all orders
    const ordersSnapshot = await db.collection("orders").get();
    const totalUserSpent = ordersSnapshot.docs.reduce((sum, doc) => {
      // Only count real money orders, not coin orders
      if (
...
// UPDATED: This no longer affects your secondary balance
